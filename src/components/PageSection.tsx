...
const PageSection = (props: PageSectionProps) => {
  ...
  return (
    <section
      ref={ref}
      id={props.id}
      className={`relative w-full overflow-x-hidden md:snap-start ${props.topBuffer ? 'pt-10 md:pt-18 ' : ''}  ${props.bottomBuffer ? 'pb-10 lg:pb-20' : ''} ${props.fullBleed ? '' : 'px-10 lg:px-20'} ${props.className}`}
    >
      {props.children}
    </section>
  );
